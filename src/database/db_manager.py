...
class DatabaseManager:
    ...
    async def store_file_metadata(self, metadata: 'FileMetadata') -> bool:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                ''', (
                    ...
                    metadata.upload_time.isoformat(),
                    metadata.owner_id,
                    json.dumps({'owner_name': metadata.owner_name}),
                    json.dumps({
    ...
    async def get_file_metadata(self, file_id: str) -> Optional['FileMetadata']:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute('''
                    SELECT hash, name, size, created_at, owner_id, permissions, metadata
                    FROM files
                    WHERE hash = ?
                ''', (file_id,)) as cursor:
                    row = await cursor.fetchone()
                    if row:
                        ...
                        return FileMetadata(
                            ...
                            name=row[1],
                            size=row[2],
                            hash=row[0],  # Using file_id as hash for now
                            owner_id=row[4],
                            owner_name=permissions.get('owner_name', 'Unknown'),
                            upload_time=datetime.fromisoformat(row[3]),
                            is_available=metadata.get('is_available', True),
                            ttl=metadata.get('ttl', 10),
                            origin_peer_id=metadata.get('origin_peer_id')
                        )
                    return None
        except Exception as e:
    ...
    async def remove_file_metadata(self, file_id: str) -> bool:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute('DELETE FROM files WHERE hash = ?', (file_id,))
                await db.commit()
                return True
        except Exception as e:
    ...
    async def get_all_file_metadata(self) -> List['FileMetadata']:
        ...
        try:
            ...
            self.logger.error(f"Error getting all file metadata: {e}")
            return [] 